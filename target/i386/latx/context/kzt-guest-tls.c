...
typedef struct kzt_guest_tls_state {
    ...
    uintptr_t inventory_generation;
    kzt_public_loader_tls_object_t
...
static kzt_public_loader_tls_object_t
    kzt_guest_tls_loader_inventory[KZT_PUBLIC_LOADER_MAX_OBJECTS];
static size_t kzt_guest_tls_loader_inventory_count;
static uintptr_t kzt_guest_tls_loader_generation;
static int kzt_guest_tls_loader_inventory_initialized;
static uintptr_t kzt_guest_tls_inventory_generation;
...
void kzt_guest_tls_loader_tracking_reset(void)
{
    ...
    memset(kzt_guest_tls_loader_inventory, 0,
           sizeof(kzt_guest_tls_loader_inventory));
    kzt_guest_tls_loader_inventory_count = 0;
    qatomic_set(&kzt_guest_tls_loader_generation, 0);
    kzt_guest_tls_loader_inventory_initialized = 0;
}
    ...
    const kzt_guest_tls_state_t *state)
{
    ...
    uintptr_t process_generation = qatomic_read(
        &kzt_guest_tls_inventory_generation);
    ...
    return state->dtv && kzt_guest_tls_epoch_can_reuse(
        ...
        pending_generation, state->inventory_generation,
        process_generation);
}
...
void kzt_guest_tls_fork_abort(void)
{
    if (latx_kzt_guest_tls_enabled() && kzt_guest_tls_fork_writer_owned) {
        kzt_guest_tls_fork_parent();
    }
}

int kzt_guest_tls_syscall_pause(CPUX86State *env)
{
    kzt_guest_tls_state_t *state = env ? env->kzt_guest_tls_allocation : NULL;

    /* Internal helpers can hold loader/inventory locks across their syscalls. */
    if (!state || state->refreshing || kzt_guest_tls_fork_writer_owned ||
        latx_guest_internal_callback_active()) {
        return 0;
    }
    /* CallbackScope keeps cancellation disabled and owns this CPU/TLS state. */
    return kzt_guest_tls_execution_pause(env);
}
...
void kzt_guest_tls_after_fork_child(CPUX86State *env)
{
    ...
    if (current) {
        ...
        kzt_guest_tls_process_inventory_count =
            current->tls_inventory_count;
        qatomic_set(&kzt_guest_tls_inventory_generation,
                    current->inventory_generation);
        memcpy(kzt_guest_tls_loader_inventory,
               current->tls_inventory,
               current->tls_inventory_count *
                   sizeof(current->tls_inventory[0]));
        kzt_guest_tls_loader_inventory_count =
            current->tls_inventory_count;
        qatomic_set(&kzt_guest_tls_loader_generation,
                    current->dtv ? current->dtv[0].value : 0);
        kzt_guest_tls_loader_inventory_initialized = 1;
        ...
        qatomic_set(&kzt_guest_tls_loader_generation, 0);
        kzt_guest_tls_loader_inventory_initialized = 0;
        qatomic_set(&kzt_guest_tls_inventory_generation, 0);
        qatomic_set(&kzt_guest_tls_pending_propagation_generation, 0);
    ...
    const kzt_public_loader_tls_object_t *right)
{
    return left->link_map_addr == right->link_map_addr &&
           ...
           left->external_registration == right->external_registration &&
    ...
    size_t object_count)
{
    ...
    for (size_t index = 0; index < object_count; ++index) {
        ...
        if (previous->image_addr != current->image_addr ||
            ...
            previous->external_registration !=
                current->external_registration ||
    ...
    int *loader_additions)
{
    ...
    *loader_additions = 0;
    for (size_t index = 0; index < object_count; ++index) {
        ...
        if (!found) {
            additions = 1;
            if (!objects[index].external_registration) {
                *loader_additions = 1;
            }
        }
    }
    ...
    size_t object_count)
{
    ...
    if (!kzt_guest_tls_process_inventory_initialized ||
        qatomic_read(&kzt_guest_tls_inventory_generation) == UINTPTR_MAX) {
        return -1;
    }
    qatomic_inc(&kzt_guest_tls_inventory_generation);
...
static int kzt_guest_tls_loader_inventory_matches(
    const kzt_public_loader_tls_object_t *objects,
    size_t object_count)
{
    size_t previous = 0;
    ...
    while (1) {
        while (previous < kzt_guest_tls_loader_inventory_count &&
               kzt_guest_tls_loader_inventory[previous]
                   .external_registration) {
            ++previous;
        }
        while (current < object_count &&
               objects[current].external_registration) {
            ++current;
        }
        if (previous == kzt_guest_tls_loader_inventory_count ||
            current == object_count) {
            return previous == kzt_guest_tls_loader_inventory_count &&
                   current == object_count;
        }
...
int kzt_guest_tls_loader_event_observe(void)
{
    kzt_public_loader_tls_object_t
        objects[KZT_PUBLIC_LOADER_MAX_OBJECTS];
    ...
    result = kzt_collect_guest_tls_objects(
        objects, KZT_PUBLIC_LOADER_MAX_OBJECTS, &object_count);
    if (result != 0 || !kzt_guest_tls_loader_inventory_initialized ||
        kzt_guest_tls_loader_inventory_matches(objects, object_count)) {
        goto out;
    }
    if (qatomic_read(&kzt_guest_tls_loader_generation) == UINTPTR_MAX) {
        result = -1;
        goto out;
    }
    memcpy(kzt_guest_tls_loader_inventory, objects,
           object_count * sizeof(*objects));
    kzt_guest_tls_loader_inventory_count = object_count;
    qatomic_inc(&kzt_guest_tls_loader_generation);
out:
    ...
    int complete_inventory)
{
    ...
    for (size_t index = 0; index < object_count; ++index) {
        ...
        /* Loader generation is updated only after complete reconciliation. */
        source->preinitialized_static_objects[
            source->preinitialized_static_count++] = *object;
    }
    return 0;
...
static int kzt_guest_tls_propagate_inventory(
    ...
    const kzt_public_loader_tls_object_t *tls_objects,
    size_t tls_object_count,
    uintptr_t generation, uintptr_t loader_generation)
{
    kzt_guest_tls_state_t
    ...
    for (size_t index = 0; index < target_count; ++index) {
        ...
        if (target->inventory_generation >= generation) {
        ...
        if (kzt_guest_tls_initialize_pending_static(
                target, tls_objects, tls_object_count, 1) != 0 ||
            ...
            target->dtv[0].value = MAX(target->dtv[0].value,
                                        loader_generation);
            target->inventory_generation = generation;
                                      ...
                                      CPUX86State *child)
{
    ...
    kzt_guest_tls_remember_inventory(
        state, tls_objects, tls_object_count);
    memcpy(kzt_guest_tls_loader_inventory, tls_objects,
           tls_object_count * sizeof(*tls_objects));
    kzt_guest_tls_loader_inventory_count = tls_object_count;
    qatomic_set(&kzt_guest_tls_loader_generation,
                state->dtv[0].value);
    kzt_guest_tls_loader_inventory_initialized = 1;
    if (!kzt_guest_tls_process_inventory_initialized) {
        qatomic_set(&kzt_guest_tls_inventory_generation, 1);
        ...
        if (changed) {
            qatomic_set(&kzt_guest_tls_pending_propagation_generation,
                        qatomic_read(&kzt_guest_tls_inventory_generation));
        }
    }
    state->inventory_generation = qatomic_read(&kzt_guest_tls_inventory_generation);
    state->refreshing = 0;
    return 0;
                                          ...
                                          int commit_inventory)
{
    ...
    if (kzt_guest_tls_initialize_pending_static(
            state, tls_objects, tls_object_count, 1) != 0) {
    ...
    inventory_has_additions = inventory_changed &&
        kzt_guest_tls_inventory_has_additions(
            state, tls_objects, tls_object_count,
            &loader_inventory_has_additions);
    state->refreshing = 1;
    ...
    if (loader_inventory_has_additions) {
        uintptr_t generation = qatomic_read(
            &kzt_guest_tls_loader_generation);

        if (!generation) {
            failure_stage = "loader-generation-snapshot";
            goto out;
        }
        state->dtv[0].value = MAX(state->dtv[0].value, generation);
    }
    process_inventory_changed = kzt_guest_tls_update_process_inventory(
        tls_objects, tls_object_count);
    ...
    propagation_generation = state->inventory_generation;
    propagation_loader_generation = state->dtv[0].value;
...
unlock:
    ...
    if (result == 0 && propagate_inventory) {
        int propagation_result = kzt_guest_tls_propagate_inventory(
            state, tls_objects, tls_object_count,
            propagation_generation, propagation_loader_generation);
...
int kzt_guest_tls_refresh(CPUX86State *env)
{
    ...
    result = kzt_guest_tls_refresh_internal(env, 1, 1);
...
int kzt_guest_tls_refresh_if_needed(CPUX86State *env)
{
    ...
    state = env->kzt_guest_tls_allocation;
    if (kzt_guest_loader_operation_active()) {
        return kzt_guest_tls_refresh_local(env);
    }
    g_rec_mutex_lock(&state->execution_lock);
    if (state->refreshing ||
...
int kzt_guest_tls_refresh_local(CPUX86State *env)
{
    ...
    kzt_guest_tls_initialize_refresh_lock();
    g_rec_mutex_lock(&kzt_guest_tls_refresh_lock);
    result = kzt_guest_tls_refresh_internal(env, 0, 0);
    g_rec_mutex_unlock(&kzt_guest_tls_refresh_lock);
    return result;
}
