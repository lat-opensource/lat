                                      ...
                                      CPUX86State *child)
{
    ...
    state->static_alignment = static_align;
...
/* Called with the state execution lock, mmap lock and states lock held. */
static int kzt_guest_tls_sample_loader_generation(
    kzt_guest_tls_state_t *state,
    const kzt_public_loader_tls_object_t *objects, size_t count,
    uintptr_t *generation)
{
    kzt_public_loader_tls_object_t observed[KZT_PUBLIC_LOADER_MAX_OBJECTS];
    size_t observed_count = 0;
    void *storage = g_try_malloc0(state->static_size + state->static_alignment - 1);
    kzt_guest_dtv_entry_t *allocation = g_try_new0(kzt_guest_dtv_entry_t,
                                                  KZT_GUEST_MAX_DTV_ENTRIES + 2);
    kzt_guest_dtv_entry_t *dtv;
    kzt_x86_64_tcbhead_t *tcb;
    uintptr_t start, tp, result;
    int status = -1;

    if (!storage || !allocation) {
        goto done;
    }
    start = ((uintptr_t)storage + state->static_alignment - 1) &
            ~(uintptr_t)(state->static_alignment - 1);
    tp = start + state->thread_pointer - state->static_start;
    tcb = (kzt_x86_64_tcbhead_t *)tp;
    dtv = allocation + 1;
    allocation[0].value = KZT_GUEST_MAX_DTV_ENTRIES;
    tcb->tcb = tp;
    tcb->self = tp;
    tcb->dtv = dtv;

    /*
     * The loader initializes a disposable TLS image, not the live thread.
     * Its resulting DTV generation covers the live slots. Do not call
     * __tls_get_addr here: allocator interposition may itself require TLS.
     * Guest loader locks must never be acquired while holding mmap/states.
     */
    g_mutex_unlock(&kzt_guest_tls_states_lock);
    mmap_unlock();
    result = RunFunctionWithStateInternalNoRefresh(
        guest_allocate_tls_init, 2, (uint64_t)tp, (uint64_t)1);
    mmap_lock();
    g_mutex_lock(&kzt_guest_tls_states_lock);
    if (result != tp || tcb->dtv != dtv) {
        goto done;
    }
    status = kzt_collect_guest_tls_objects(observed,
        KZT_PUBLIC_LOADER_MAX_OBJECTS, &observed_count);
    if (status != 0) {
        goto done;
    }
    if (observed_count != count) {
        status = KZT_GUEST_TLS_REFRESH_BUSY;
        goto done;
    }
    for (size_t i = 0; i < count; i++) {
        if (!kzt_guest_tls_same_object_ignoring_module(&objects[i], &observed[i]) ||
            (observed[i].module_id && observed[i].module_id != objects[i].module_id) ||
            !dtv[objects[i].module_id].value) {
            status = KZT_GUEST_TLS_REFRESH_BUSY;
            goto done;
        }
    }
    /* Removed slots may make a fresh DTV older than this thread's DTV. */
    *generation = MAX(state->dtv[0].value, dtv[0].value);
    status = 0;
done:
    g_free(allocation);
    g_free(storage);
                                          ...
                                          int commit_inventory)
{
    ...
    if (inventory_has_additions) {
        uintptr_t generation;
        int sample_result = kzt_guest_tls_sample_loader_generation(
            state, tls_objects, tls_object_count, &generation);
