...
static __thread unsigned int callback_internal_depth;

int latx_guest_internal_callback_active(void)
{
    return callback_internal_depth != 0;
}
...
static void callback_scope_leave(CallbackScope *scope)
{
    ...
    if (current.internal_entered) {
        g_assert(callback_internal_depth > 0);
        --callback_internal_depth;
    }
