...
"$clang" --target=x86_64-linux-gnu -fuse-ld=lld -nostdlib -static \
    ...
    -o "$workdir/softfpu-region-planner-x86_64"

LATX_AOT=0 LATX_MT=0 LATX_SOFTFPU=1 LATX_SOFTFPU_FAST=0 \
    "$emulator" "$workdir/softfpu-region-planner-x86_64"

for fast in 0 0x40 0xffffff; do
    LATX_AOT=0 LATX_MT=0 LATX_SOFTFPU=2 LATX_SOFTFPU_FAST=$fast \
        "$emulator" "$workdir/softfpu-region-planner-x86_64"
done

echo "PASS: softfpu region planner handles transparent and local fallbacks"
